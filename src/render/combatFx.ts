...
import { TEAM_COLORS, resolveModelCfg } from "./units";
...
export async function createCombatFx(scene: Scene): Promise<CombatFx> {
  // arrow prototype
  let arrowProto: Mesh = MeshBuilder.CreateCylinder("arrowProto", { height: 1.1, diameter: 0.16, tessellation: 6 }, scene);
  ...
  // upgrade to the real fletched arrow model (KayKit) once loaded
  void SceneLoader.ImportMeshAsync("", `${import.meta.env.BASE_URL}models/`, "arrow.gltf", scene).then((r) => {
    const mesh = r.meshes.find((m): m is Mesh => m.getTotalVertices() > 0) as Mesh | undefined;
    if (!mesh) return;
    const bounds = mesh.getHierarchyBoundingVectors();
    const len = Math.max(bounds.max.z - bounds.min.z, bounds.max.x - bounds.min.x, bounds.max.y - bounds.min.y, 0.001);
    mesh.scaling.setAll(1.1 / len);
    mesh.bakeCurrentTransformIntoVertices();
    mesh.isVisible = false;
    mesh.isPickable = false;
    arrowProto = mesh;
  }).catch(() => { /* keep the glowing bolt fallback */ });
  ...
  // death pose pools per model file (lazy — first death of a type warms it)
  const deathPools = new Map<string, Mesh[] | "loading">();
  const loadDeathPool = async (file: string, height: number, deathRe: RegExp, loadout: RegExp | null): Promise<void> => {
    const result = await SceneLoader.ImportMeshAsync("", `${import.meta.env.BASE_URL}models/`, file, scene);
    const root = result.meshes[0]!;
    const bounds = root.getHierarchyBoundingVectors();
    root.scaling.setAll(height / Math.max(0.001, bounds.max.y - bounds.min.y));
    const pose = result.animationGroups.find((g) => deathRe.test(g.name)) ?? result.animationGroups[0];
    ...
    const meshes = result.meshes.filter((m): m is Mesh => m.getTotalVertices() > 0 && (!loadout || loadout.test(m.name))) as Mesh[];
    for (const m of result.meshes) {
      if (m.getTotalVertices() > 0 && loadout && !loadout.test(m.name)) m.setEnabled(false);
    }
    for (const m of meshes) m.isVisible = false;
    deathPools.set(file, meshes);
  };
  await loadDeathPool("villager.glb", 1.05, /Death_A$/, null);
  await loadDeathPool("knight.glb", 1.15, /Death_A$/, /^(Knight_(Arm|Body|Head|Leg|Helmet|Cape).*|1H_Sword|Round_Shield)$/);
  ...
  const collect: CombatFx["collect"] = (events, groundHeightAt) => {
    ...
    for (const d of events.deaths) {
      ...
      if (!pool || pool === "loading") {
        if (!pool) {
          deathPools.set(cfg.file, "loading");
          void loadDeathPool(cfg.file, cfg.height, cfg.death, cfg.loadout);
        }
        continue;
      }
      void TEAM_COLORS;
